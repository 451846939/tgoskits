...
已验证主链路为 Linux（2 vCPU）到 ArceOS 或 FreeRTOS（各 1 vCPU）的 AICP v1 over TCP/IP：Linux
...
可通过 `aicp.sh run linux freertos` 复现。RT-Thread 和 Zephyr 的旧网络 Guest
仍依赖 legacy VirtIO-MMIO 直通设备，不能与当前上游设备模型混用；它们不在
`aicp.sh run`、`smoke`、`full` 或 PR CI 中声明为已验证闭环，原生周期基线仅用于
任务一对照。
