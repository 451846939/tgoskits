...
fn init_allocator() {
    ...
    // Initialize from the largest free range so the allocator has enough space
    // for its first metadata section, then add every remaining firmware range.
    // Both page and byte allocations can use all successfully added sections.
...
fn init_interrupt() {
    ...
    #[cfg(feature = "ipi")]
    {
        ...
        ax_hal::cache::enable_remote_tlb_shootdown();
    }
}
