...
pub fn process_mem_stats_formats_linux_fields() -> bool {
    ...
    let shared_rss = stats.rss_file_pages + stats.rss_shmem_pages;
    let statm = alloc::format!(
        "{} {} {} {} 0 {} 0\n",
        stats.vss_pages,
    ...
    statm == "256 48 8 10 0 64 0\n"
        && stats.vsize_bytes() == 256 * 4096
        && stats.rss_pages() == 48
        && {
            let status = super::pseudofs::proc::format_status_vm_lines(&stats);
...
pub fn memory_accounting_tracks_cow_charge_transitions() -> bool {
    ...
    acct.inc(RssKind::File, 1).is_ok()
        && acct.reclassify(RssKind::File, RssKind::Anon, 1).is_ok()
        && acct.snapshot_resident_pages() == (1, 0, 0)
        && acct.dec(RssKind::Anon, 1).is_ok()
        && acct.snapshot_resident_pages() == (0, 0, 0)
}
...
pub fn memory_accounting_rejects_duplicate_and_conflicting_charges() -> bool {
    ...
    let acct = MemoryAccounting::new();
    acct.inc(RssKind::File, 1).is_ok()
        && acct.dec(RssKind::File, 2).is_err()
        && acct.snapshot_resident_pages() == (0, 1, 0)
}
...
pub fn accounting_edge_cases_and_snapshot_rules_hold() -> bool {
    ...
    let acct = MemoryAccounting::new();
    acct.inc(RssKind::Anon, 0).is_ok()
        && acct.dec(RssKind::File, 0).is_ok()
        && acct.inc(RssKind::Anon, 5).is_ok()
        && acct.inc(RssKind::File, 2).is_ok()
        && acct.snapshot_resident_pages() == (5, 2, 0)
        && acct.hiwater_rss_pages() == 7
}
...
pub fn rss_kind_and_accounting_rules_hold() -> bool {
    ...
    let acct = MemoryAccounting::new();
    acct.inc(RssKind::Anon, 1).is_ok()
        && acct.inc(RssKind::File, 2).is_ok()
        && acct.inc(RssKind::Shmem, 3).is_ok()
        && acct.rss_total_pages() == 6
        && alloc::format!("{:?}", RssKind::Anon) == "Anon"
}

pub fn accounting_rss_kind_debug_and_default_hold() -> bool {
    use super::mm::{MemoryAccounting, RssKind};

    let acct = MemoryAccounting::default();
    acct.rss_total_pages() == 0
        && RssKind::File == RssKind::File
        && alloc::format!("{:?}", RssKind::Shmem) == "Shmem"
}

pub fn process_vm_stat_watermarks_hold() -> bool {
    starry_mm::process_vm_stat_watermarks_hold_for_test()
}

pub fn process_vm_stat_edge_cases_hold() -> bool {
    starry_mm::process_vm_stat_edge_cases_hold_for_test()
}
...
pub fn stats_classify_and_accumulate_rules_hold() -> bool {
    ...
    use starry_mm::{ProcessMemStats, ResidentSnapshot, StackRange, VmaPermissions};

    let stack = StackRange {
        start: VirtAddr::from(0x8000usize),
        end: VirtAddr::from(0xa000usize),
    };
    let mut stats = ProcessMemStats::default();
    stats.record_vma(
        stack,
        (VirtAddr::from(0x1000usize), VirtAddr::from(0x3000usize)),
        VmaPermissions {
            writable: false,
            executable: true,
        },
    ...
    stats.record_vma(
        stack,
        (VirtAddr::from(0x8000usize), VirtAddr::from(0xa000usize)),
        VmaPermissions {
            writable: true,
            executable: false,
        },
