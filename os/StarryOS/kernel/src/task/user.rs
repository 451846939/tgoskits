...
fn handle_user_page_fault(
    ...
    let signal = {
        ...
        match aspace.handle_page_fault(address, flags) {
            ...
            starry_mm::FaultOutcome::PermissionDenied => Some(SignalInfo::new_fault(
                Signo::SIGSEGV,
                SEGV_ACCERR,
                address.as_usize(),
            )),
            starry_mm::FaultOutcome::BackingError => Some(SignalInfo::new_fault(
                Signo::SIGBUS,
                BUS_ADRERR,
                address.as_usize(),
            )),
            starry_mm::FaultOutcome::NoMemory => Some(SignalInfo::new_kernel(Signo::SIGKILL)),
        }
    };
    if let Some(signal) = signal {
        ...
        raise_signal_fatal(signal, context).expect("failed to deliver fatal page-fault signal");
    }
}
