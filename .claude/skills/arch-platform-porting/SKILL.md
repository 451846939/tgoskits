...
- **Axvisor guest platform identity**: every Axvisor guest machine has exactly one machine-owned virtual UART; guest TOML must not select its model, address, IRQ, backend, or enabled state. On firmware-backed hosts, replacing a physical platform device with an emulated implementation must preserve the firmware-visible identity: derive the virtual UART node path, phandle, address span, register model/shift/access width, interrupt-parent, and IRQ specifier from FDT `stdout-path`, and place the vGIC distributor/redistributor at the host GIC windows while retaining its phandle. Keep architecture profile values as the fallback for fixed machines such as QEMU. The corresponding physical UART and GIC ranges remain host-owned and must be removed from both explicit and default device-passthrough mapping even though the virtual devices trap the same guest addresses. Axvisor's application-layer console mux is the sole host-console input reader. Its `SerialBackendFactory` must create a fresh per-device-generation backend whenever the VM device graph is rebuilt; stop/delete invalidates the old generation so retained callbacks cannot read from or write to a replacement VM. A host-derived UART INTID remains a virtual IRQ line routed through the guest controller; matching the number does not enable physical IRQ passthrough.
...
- **Page-table ownership**: keep host Stage-1 formats, walkers, and invalidation metadata in `ax-cpu::paging`; `ax-hal` only supplies runtime frames and remote shootdown integration. Keep guest Stage-2 adapters in `axvm` and boot adapters in `someboot`. `page-table-core` must remain architecture-neutral and must not select implementations with `target_arch`.
