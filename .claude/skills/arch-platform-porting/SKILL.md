...
- **Axvisor guest console ownership**: every Axvisor guest machine has exactly one architecture-fixed virtual UART; guest TOML must not select its model, address, IRQ, backend, or enabled state. The host physical UART remains host-owned and must be removed from both explicit and default device-passthrough discovery. Axvisor's application-layer console mux is the sole host-console input reader and supplies per-VM byte backends to the virtual UARTs. Route UART level interrupts through the guest IOAPIC, vGIC, vPLIC, or PCH-PIC/EIOINTC path; device passthrough must not implicitly enable physical IRQ passthrough.
