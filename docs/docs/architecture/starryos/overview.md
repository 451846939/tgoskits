...
| Starry 专用组件层 | `components/starry-*`、`memory/starry-mm` | 进程、信号、用户访问边界和 Linux VM policy |
...
| mm | `kernel/src/mm/*` | 具体地址空间、页表/VFS backend、ELF 装载和 mmap ABI 接线 | `starry-mm`、`starry-vm`、`ax-mm` |
...
- **starry-mm** — OS 无关的 Linux VM policy、RSS/VSS/commit 记账、COW 引用规则、共享页 owner、fault/reclaim 决策和 capability 数据。
- **starry-vm** — 用户地址检查和 copy 边界，不拥有 `AddrSpace` 或 Linux VMA policy。

具体 `AddrSpace` 与 anonymous/linear/COW/file/shared `PageTableCursor` backend 保留在 kernel adapter，避免为抽离 VFS listener、memfd 和页表锁再增加一层通用框架。
