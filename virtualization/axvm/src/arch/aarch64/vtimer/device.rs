...
impl VirtualTimerBank {
    pub(super) fn new(vgic: Arc<VgicCore>, vcpu_count: usize, ppi: PpiId, frequency: u64) -> Self {
        Self {
            timers: (0..vcpu_count)
                .map(|vcpu| {
                    Arc::new(VirtualPhysicalTimer::new(
                        vgic.clone(),
                        GicVcpuId::new(vcpu),
                        ppi,
                        frequency,
    ...
    fn timer_for_current_vcpu(&self) -> Result<&Arc<VirtualPhysicalTimer>, DeviceError> {
        let vcpu = crate::current_vcpu_id().ok_or_else(|| DeviceError::InvalidState {
            operation: "access AArch64 architectural timer",
    ...
    fn write(&self, addr: u32, value: u64) -> Result<(), DeviceError> {
        ...
        result.map_err(|error| DeviceError::Backend {
            operation: "update AArch64 architectural timer interrupt",
